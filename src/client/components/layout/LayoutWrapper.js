...
export default function LayoutWrapper({ children }) {
	// Conditionally use Auth0's useUser only when NOT in selfhost mode
	const isSelfHost = process.env.NEXT_PUBLIC_ENVIRONMENT === "selfhost"
	const auth0Result = isSelfHost
		? { user: null, error: null, isLoading: false }
		: useUser()

	const { user, error: authError, isLoading: isAuthLoading } = auth0Result
	const {
		isSearchOpen,
		isMobileNavOpen,
		openSearch,
	...
	useEffect(() => {
		// In selfhost mode, bypass Auth0 authentication checks
		const isSelfHost = process.env.NEXT_PUBLIC_ENVIRONMENT === "selfhost"
